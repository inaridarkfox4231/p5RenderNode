...
// いろいろ上手くいってるのでこの状態で保存して、本番は次でやります。
...
void main(){
  ...
  vec2 texCoord = texture(uUvMap, p).xy;
  vColor = vec4(1.0);
...
function setup(){
  ...
  _node.registFBO("defer", {w:w, h:h, color:{info:[{}, {type:"float"}, {type:"float"},
                  {type:"float", internalFormat:"rg32f", format:"rg", magFilter:"nearest"}]}});
  _node.registFBO("customTexture", {w:1024, h:1024, color:{info:{}}}); // ここになんか描き込む

  _node.bindFBO("customTexture").clearColor(0, 0.5, 1, 1).clear().bindFBO(null);
...
function draw(){
  ...
  render(_node, _tf, _cam, 0, 0, 0); // deferに各種情報を格納
