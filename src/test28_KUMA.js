...
// density:0.5～1.0で0.01刻み
// sizeFactor:0.1～1.0で0.01刻み
// baseColor:色々
// そんな感じでlilでいじる。
...
const config =
{
  ...
  clear: function(){
    const col = config.clearColor;
    _node.bindFBO("customTexture").clearColor(col.r, col.g, col.b).clear();
  }
}

function createGUI(){
  const gui = new lil.GUI();
  gui.addColor(config, "brushColor").name("color");
  gui.add(config, "sizeFactor", 0.1, 1.0, 0.01).name("size");
  gui.add(config, "density", 0.5, 1.0, 0.01);
  gui.addColor(config, "clearColor");
...
uniform sampler2D uGraphic; // グラフィックから色を採取する。
in vec2 aPosition;
in vec2 aGraphicCoord;
uniform vec2 uMouse;
out vec4 vColor;
void main(){
  vec2 p = uMouse + aPosition;
  vec3 texCoord = texture(uUvMap, p).xyz;
  vColor = texture(uGraphic, aGraphicCoord);
...
void main(){
  ...
  d = pow(d, 4.0);
  float alpha = d*(1.0-r);
  brushColor = vec4(uBaseColor, alpha);
}
`;
...
function registKumaleon(node, size){
  ...
  for(let i=0; i<N; i++){
    ...
    const n = kumaleon.vertexNormals[i];
    normals[3*i] = n.x;
    normals[3*i+1] = n.y;
    normals[3*i+2] = n.z;
...
function registPoints(node, r, w, h){
  ...
  for(let x=-r; x<r; x++){
    for(let y=-r; y<r; y++){
      ...
      uv.push((x+r)*0.5/r, (y+r)*0.5/r);
    }
  }
  node.registFigure("points", [{name:"aPosition", size:2, data:v}, {name:"aGraphicCoord", size:2, data:uv}]);
}
...
function drawGraphic(){
  ...
  _node.use("airBrush", "foxBoard")
       ...
       .setUniform("uSizeFactor", config.sizeFactor)
       .setUniform("uBaseColor", [col.r, col.g, col.b])
...
function texturePaint(){
  ...
  _node.enable("blend").blendFuncSeparate("src_alpha", "one_minus_src_alpha", "one", "one");

  _node.use("paint", "points")
       .setFBOtexture2D("uUvMap", "defer", "color", 3)
       .setFBOtexture2D("uGraphic", "airBrush")
...
function setup(){
  ...
  // 描画用グラフィック。もうエアブラシ限定でいいや。
  _node.registFBO("airBrush", {w:256, h:256, color:{info:{}}});
  // 描画用シェーダ
  _node.registPainter("airBrush", airBrushVert, airBrushFrag); // レガシー
  // 描画用GUI
  createGUI();
}
...
function draw(){
  ...
  ex.copyPainter(_node, {src:{type:"fb", name:"customTexture", view:[0.75, 0.75, 0.25, 0.25]}})
