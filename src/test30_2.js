...
// じゃあスキャンライン、やろうか。
// グローバルポジションを使うんだって
// 幅は色部分の割合をユニフォームで指定、色もユニフォームで指定、移動スケールもユニフォームで指定、するだけ。
// globalはmodelMatrixを使って算出するのでそのための行列が要る。
...
void main(void){
  ...
  vGlobalPosition = (uModelMatrix * vec4(aPosition, 1.0)).xyz;
...
// scanLine.
uniform int uScanLineCount;
uniform vec3 uScanLineColor[3];
uniform vec3 uScanLineNormal[3];
...
uniform bool uUseScanLine; // スキャンラインフラグ
...
in vec3 vGlobalPosition; // これを使ってスキャンラインカラーを計算する
...
// ----------------------- スキャンライン色 ------------ //
float getScanLineAlpha(vec3 p, vec3 n, float diff, float segment, float ratio){
  // pとnの内積を取る
  // diffを引く
  // 結果をsegment（ラインのセグメント幅）で割ってfractを取る
  // さらに処理を加えて途中まで0となるようにする, ratioを使う
  // 出力。
  float k = dot(p, n) - diff;
  k = fract(k / segment);
  k = max(0.0, (k - (1.0 - ratio)) / ratio);
  return k;
}
...
void main(void){
  ...
  // スキャンライン
  if(uUseScanLine){
    for(int i=0; i < uScanLineCount; i++){
      float sAlpha = getScanLineAlpha(vGlobalPosition, uScanLineNormal[i], uTime, 1.0, 0.5);
      result = (1.0 - sAlpha) * result + sAlpha * uScanLineColor[i];
    }
  }
...
function setModelView(node, tf, cam){
  ...
  node.setUniform("uModelMatrix", modelMat)
      .setUniform("uViewMatrix", viewMat)
...
function draw(){
  ...
  _node.setUniform("uUseScanLine", false);
  _node.drawFigure("plane").bindIBO("planeIBO");
  renderMesh(_node, _tf, _cam, 0,0,0, 0.6, 1.0, 0.8);

  _node.setUniform("uUseScanLine", true);
  _node.setUniform("uScanLineCount", 3);
